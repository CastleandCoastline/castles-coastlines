...
const GuestLogin = ({ tours, onUnlock, onGuideLogin }) => {
  ...
  const tryUnlock = () => {
    ...
    if (match) { onUnlock(match); }
    else { setError("That code doesn't match any tour. Please check with your guide."); setShake(true); setTimeout(() => setShake(false), 500); }
  };
  return (
          ...
          {tour.guide_name && (
            <div style={{ background: "#1a2332", borderRadius: 16, padding: 20, border: "1px solid #c9a96e20", display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, fontWeight: 700, color: "#f0e6d3" }}>{tour.guide_name}</div>
              </div>
            </div>
          )}
          {tour.guide_phone && (
      ...
      {!notes ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {paragraphs.map((para, i) => (
            <div key={i} style={{ background: "#1a2332", borderRadius: 14, padding: "16px 18px", border: "1px solid #ffffff10", display: "flex", gap: 12, alignItems: "flex-start" }}>
...
const AddTourModal = ({ onSave, onClose, saving }) => {
  ...
  const handleSave = () => {
    ...
    onSave({ id, name, duration: parseInt(duration), description: desc, password: password.toUpperCase(), announcement: "", notes: "", guide_name: "", guide_phone: "", guide_email: "", days: [] });
  };
  return (
...
const TourSettingsModal = ({ tour, onSave, onClose, saving }) => {
  ...
  const inp = (label, val, fn, ph, type = "text") => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
      <label style={{ fontSize: 11, color: "#c9a96e", letterSpacing: 1, textTransform: "uppercase" }}>{label}</label>
      <input value={val} onChange={(e) => fn(e.target.value)} placeholder={ph} type={type} style={{ background: "#0d1520", border: "1px solid #ffffff20", borderRadius: 8, padding: "10px 12px", color: "#f0e6d3", fontSize: 14, width: "100%", outline: "none" }} />
    </div>
  );
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 1000, overflowY: "auto", padding: "20px 16px" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#0d1520", fontFamily: "'Lato',sans-serif", color: "#f0e6d3" }}>
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        <div style={{ background: "#1a2332", borderRadius: 16, padding: 20, marginBottom: 16, border: "1px solid #c9a96e20" }}>
          ...
          {editingPassword ? (
            <div style={{ display: "flex", gap: 8 }}>
              <input value={passwordDraft} onChange={(e) => setPasswordDraft(e.target.value.toUpperCase())} maxLength={12} style={{ flex: 1, background: "#0d1520", border: "1px solid #c9a96e40", borderRadius: 8, padding: "9px 12px", color: "#f0e6d3", fontSize: 16, fontFamily: "monospace", letterSpacing: 3, outline: "none" }} />
